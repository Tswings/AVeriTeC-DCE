...
def load_text(args, source_fp, target_fp, device):
    ...
    def select_candidate_sentences_task1b(sample):
        """
        ...
        return sent_ids_select_by_simcse, sentences
    ...
    if (target_fp == ''):
        samples = json.load(open(source_fp, 'r'))       # [:10]
        for sample in samples:
            fulltext = sample['fulltext']
            if fulltext[0] in ["“", "'", "”"] and fulltext[-1] in ["“", "'", "”"]:
                fulltext = fulltext[1:-1]

            # select candidate sentences    # dzy
            cand_sent_ids, sents = select_candidate_sentences(fulltext)
            # cand_sent_ids, sents = select_candidate_sentences_task1b(sample)
            ...
            batch.cand_sent_ids = cand_sent_ids     # dzy
...
def load_text_ori(args, source_fp, target_fp, device):
    from others.tokenization import BertTokenizer
    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True)
    sep_vid = tokenizer.vocab['[SEP]']
    cls_vid = tokenizer.vocab['[CLS]']
